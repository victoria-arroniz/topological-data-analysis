...
positional way — the way `DataAsymp`/`DataSymp` are built in `funTDA.R`
prepends (`cbind(x1, Data)`) each new network to those already accumulated,
reversing the order within each group relative to the file ID, so the
"S1..S9" label should not be read as the subject's real ID without separately
verifying it) overlaps with the asymptomatic region. PC1 separates the two
groups reasonably well; PC2 shows no clear separation.
