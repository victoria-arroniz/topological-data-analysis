...
committed — none of the `.qmd` reports or the README embed anything from `figures/`,
so there is nothing that depends on it staying in sync, and committing it once
already let it silently go stale against a later change to `make_figs.py`. Run
`python make_figs.py` to (re)generate the 5 figures locally.
