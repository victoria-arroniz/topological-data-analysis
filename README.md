...
landscapes — which is two steps removed from the image, carries no direct identifier,
...
The `requirements.txt` at the repo root covers `make_figs.py` alone, which computes no
homology and so needs neither persistent-homology stack. There is deliberately no shared
file for the three parts: Part A and Part B need mutually incompatible numpy versions
(`numpy<2.0` for Part A vs. `numpy>=1.24` for Part B — see the explanation under Part A
below), so one would promise a common environment that can't actually exist. Install each
part's `requirements.txt` separately, ideally in its own virtual environment.
