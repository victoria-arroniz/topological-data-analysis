...
ref <- list(
  # Re-baselined 3 Aug 2026. The committed output/*.csv had drifted from the
  # notebook that is supposed to produce them: the CSVs dated from 29 July,
  # while beetles_landscapes.ipynb -- which has a fixed seed and is exactly
  # reproducible -- produces the 30 July .npy. Re-running the notebook today
  # reproduces the .npy bit for bit and does not reproduce the old CSVs, so the
  # CSVs were the stale ones and were regenerated from the .npy. Everything
  # below moved by the corresponding amount; the separation between the two
  # regimes is unaffected.
  n_stable_zero = 126, n_aperiodic_zero = 0,
  mean_max_stable = 0.0020, mean_max_aperiodic = 0.228,
  pc1_pct = 84, pc2_pct = 13,
  # Was 42 before the 3 Aug re-baselining and is 45 after. It was missed
  # when the other baselines were updated: the assert below tolerates a
  # drift of 5, so |45 - 42| = 3 passed and the stale value survived.
  n_aperiodic_pc1_neg = 45,
  # test1_Tobs was 85.5 before the 3 Aug re-baselining and is 68.05 after, a
  # move of 17.4 that broke the tolerance of 15 on the first re-render. The
  # test result is untouched: crit95 moved by 0.04 against a tolerance of 0.5,
  # the p-value is still 0, and Tobs remains 25 times the critical value.
  # What moved is only where the supremum lands. It sits at t = 0.168, where
  # the mean difference between regimes is 0.109, rather than at t = 0.283,
  # where the difference is largest at 0.176 but the statistic is only 37.4.
  # The pooled variance at the first point is 8.6 times smaller than at the
  # second, so the studentised supremum is bought by a small local variance
  # and not by a large difference. That is the sensitivity the loose tolerance
  # was meant to absorb, and it was not loose enough. Tobs is deterministic
  # given the landscapes, so it should now be stable across re-renders.
  test1_Tobs = 68.05, test1_crit95 = 2.75,
  ...
  #
  # Was 3500 before the 3 Aug re-baselining and is 1926 after. The move is
  # arithmetic, not a problem: var_stable sits in the denominator, and the
  # regenerated CSVs carry 74 non-zero stable landscapes rather than 71, with
  # the mean stable peak rising from 0.0016 to 0.0020. A larger denominator is
  # a smaller ratio. var_aperiodic is unchanged at 3.43e-03.
  #
  # The exchangeability argument below is unaffected. Three orders of
  # magnitude of dispersion between the two samples is still three orders of
  # magnitude, and the caveat about the validity of the p-value stands as
  # written. Only the figure quoted in that paragraph changes; see the note
  # there, where "three and a half orders" was corrected to match log10(1926)
  # = 3.28. The quantity is deterministic given the landscapes, so it should
  # not move again unless they do.
  var_ratio = 1926,
  # With the transient retained, the stable regime is NOT uniformly zero (74
...
# of the make_figs.py palette (COL_BLUE_LIGHT / COL_RED_LIGHT), which is what
# ts_landscapes_overlaid.png is drawn in, so the ggplot figures of this report
# and the matplotlib ones next to them now match. Change them here, not at the
# point of use.
col_stable    <- "#1a5fa8"
col_aperiodic <- "#C0392B"
...
stopifnot(
  "Expected 126 of 200 stable landscapes identically zero on the grid (a grid-resolution effect, see the transient section below, not an empty H1 diagram)" =
...
# burn-in variant — do not collapse onto a single point, since 74 of them
...
PC1 < 0 half-plane — unsurprising, since 126 of them share the exact same
...
This is not a limitation of the FDA pipeline: for some aperiodic
trajectories the reconstructed attractor stays close enough to a compact
blob that its H1 landscape resembles a stable series’ near-zero landscape.
It is tempting to blame the fixed embedding parameters ($m = 2$,
$\tau = 3$), since Takens’ theorem guarantees only that a *generic*
embedding recovers the attractor topology, and $(m, \tau)$ are constants
here rather than tuned per series. That explanation has been tested and
does not hold. `embedding_sweep.py` crosses $m \in \{2, 3\}$ with the
retained transient: raising $m$ to the value Takens’ bound requires moves
the overlap from 45/200 only to 40/200, while discarding the transient
removes most of it at either dimension (9/200 and 10/200). What these
borderline series carry is a retained approach to the attractor, not too
low an embedding dimension. A sweep over $\tau$ remains untested.
...
final statistic is the supremum over $t$. Because 126 of the 200 stable
...
vtperm <- function(X, i1, i2, nperm = 10000) {
  ...
  Ts <- function(M) {
    ...
    # analysis below: with the transient retained, 74 of the 200 stable
...
samples differing by more than three orders of magnitude in dispersion are
...
# degenerate — the stable regime's 74 non-zero landscapes give its control
...
stable-vs-aperiodic separation is significant at `p < 1e-4` with `r n_aperiodic_pc1_neg` of 200
