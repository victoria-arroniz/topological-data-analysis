...
2. **The original `Example1Beetles.R` does not run this functional hypothesis
...
stopifnot(
  "Expected exactly 129 stable series with an identically-zero landscape" =
    n_stable_zero == 129,
...
`Example1Beetles.R` never asked this question in a functional-hypothesis-testing
...
splithalf_summary <- data.frame(
  ...
  `mean p-value` = sprintf("%.3f", c(mean(pvalsStable), mean(pvalsAperiodic))),
  `sd(p-value)` = sprintf("%.3f", c(sd(pvalsStable), sd(pvalsAperiodic))),
  `% splits with p < 0.05` = sprintf("%.1f%%", c(mean(pvalsStable < 0.05), mean(pvalsAperiodic < 0.05)) * 100),
  check.names = FALSE
)
...
stopifnot(
  "Stable control mean p implausible"    = abs(mean(pvalsStable) - 0.5) < tol_mean + 0.1,
  "Aperiodic control mean p implausible" = abs(mean(pvalsAperiodic) - 0.5) < tol_mean + 0.1,
  "Stable control frac<0.05 far from nominal 5%"    = abs(mean(pvalsStable < 0.05) * 100 - 5) < tol_frac + 3,
...
|---|---:|---:|---:|
| Stable | `r sprintf("%.3f", mean(pvalsStable))` | `r sprintf("%.3f", sd(pvalsStable))` | `r sprintf("%.1f%%", mean(pvalsStable < 0.05) * 100)` |
