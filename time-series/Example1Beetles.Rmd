...
> **Note:** This is the original, heritage code for the Part B (Tribolium
> beetle) analysis, kept here as reference only. It is not maintained and is
> not part of the reproducible pipeline; see `results_partB.qmd` for the
> validated FPCA + permutation-test analysis.
>
> Requires the R packages `deSolve`, `tidyverse`, `data.table`, `caret`,
> `nonlinearTseries`, `TDAstats`, and `matrixStats`. This document has not
> been rendered in the current environment (packages not installed there).
